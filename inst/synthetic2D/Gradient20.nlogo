...
to setup-grid
  ...
  ;; NOW SET UP THE SETTINGS
    ask cells
    [set MaxNumberRows 3           ;; parameter that determine the Max number of rows that it propagates
     set ERKPulseDuration 7        ;; parameter that tells how long a ERk pulse lasts (5 minutes time interval)
     set SurvivalDuration 0       ;; parameter that tells how long the survival lasts (5 minutes time interval)
     set ApoptosisColorDuration 7  ;; duration of apoptosis red color (only for visualization, in 5 minutes interval)
     set ReplacementDuration 7     ;; time that is needed for an apoptotic cell to be replaced (5 minutes time interval)
     set SurvivalProbability 70    ;; parameter that tells the probability to survive of a cell in survival mode (expressed in percentage)
     set PropagationError 15       ;; percentage of cells that fail to propagate the wave
     set ApoptoticRate 0          ;; apoptotic rate per 5 minutes (in 100,000)
     set NoApoSourceProbability 20 ;; NoApoSource rate per 5 minutes (in 100,000)
     set RefractoryTime 7         ;; the time in which a cell cannot be re-activated. Important for directional propagation of the waves. Typically the duration of the ERK pulse (in 5 minutes)
     set MinActiveNeighbors 1]     ;; the minimum number of active neighbors that is required to propagate the wave
  end
...
to go
  ...
  ask cells
    [ if random 100 <= SurvivalProbability and Survival = 1 and Apoptosis = 1
        [ set apoptosis 0] ]
  ask cells
    [ if random 100000 < NoApoSourceProbability and countdownReplacement = 0
        [ set NoApoSource 1] ]
  ...
  ask cells
    [ if countdownERK = 0 and Survival = 0
      [ set ERKactivityValue 0]]
  ...
  csv:to-file word ticks ".csv" [ (list ticks who xcor ycor apoptosis ERKactivity Survival ERKactivityValue) ] of turtles

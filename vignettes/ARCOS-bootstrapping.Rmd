...
lPar$nboot = 1000L # number of bootstrapping iterations
...
ggplot(tFracActInCollBoot,
  ...
  geom_vline(xintercept = tFracActInColl$fracAct,
             color = "red", linetype = 2) +
  xlab('Fraction of activity in collective events') +
  ggthemes::theme_clean()
```

P-value, alternative hypothesis - the fraction of total (binarised) activity in collective events from bootstrapping is lower or greater than the observed fraction:

```{r}
calcPvalFromBS(tFracActInColl$fracAct, tFracActInCollBoot$fracAct, corrected = T, alternative = 'two.sided')
```
...
P-value, alternative hypothesis - the number of clusters from bootstrapping is lower or greater than the observed number:
...
P-value, alternative hypothesis - mean cluster duration from bootstrapping is lower or greater than the observed statistic:
...
P-value, alternative hypothesis - mean total cluster size from the bootstrap is lower or greater than the observed statistic:
